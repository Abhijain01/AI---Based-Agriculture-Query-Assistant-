...
# Optional translator (handle missing gracefully)
try:
...
# NOTE: The manual @app.after_request CORS handler has been removed
# as it is redundant with the Flask-CORS extension above.
...
# **FIXED**: Replaced `@app.before_first_request`
# This starts the model loading in a background thread as soon as the app module is imported.
...
def ask_gemini_once(prompt_en: str) -> str:
    ...
    try:
        ...
        return resp.text if getattr(resp, "text", None) else "No response from Gemini."
    except Exception as e:
...
def recommend_fertilizer():
    ...
    try:
        ...
        # **IMPROVED**: Return a generic error to avoid leaking implementation details.
...
def transcribe_audio():
    ...
    # **IMPROVED**: Use try...finally to guarantee temporary file cleanup.
    ...
    try:
        ...
        text = ""
        try:
            uploaded = genai.upload_file(path=tmp_wav_path)
            speech_model = genai.GenerativeModel("gemini-1.5-flash")
        ...
        # This cleanup block will always run, even if errors occur.
